...
public class ElongationTransition: NSObject {
    ...
    fileprivate func root(from context: UIViewControllerContextTransitioning) -> DesignViewController {
        let viewController = context.viewController(forKey: rootKey)

        if let navi = viewController as? UINavigationController {
            for case let elongationViewController as DesignViewController in navi.viewControllers {
                return elongationViewController
            }
        } else if let tab = viewController as? UITabBarController, let elongationViewController = tab.selectedViewController as? DesignViewController {
            return elongationViewController
        } else if let elongationViewController = viewController as? DesignViewController {
            return elongationViewController
        }

        fatalError("Can't get `DesignViewController` from UINavigationController nor from context's viewController itself.")
    }

    fileprivate func detail(from context: UIViewControllerContextTransitioning) -> DetailDesignViewController {
        return context.viewController(forKey: detailKey) as? DetailDesignViewController ?? DetailDesignViewController()
    }
}
...
extension ElongationTransition {
    ...
    fileprivate func present(using context: UIViewControllerContextTransitioning) {
        ...
        let root = self.root(from: context) // DesignViewController
        let detail = self.detail(from: context) // DetailDesignDetailViewController
